...
class InfixAddOnsController extends Controller
{
    ...
    public function ManageAddOns()
    {
        try {
            ...
            return view('systemsetting::module_manage', compact('is_module_available', 'module_list'));
        } catch (Throwable $th) {
        ...
        if (Config::get('app.app_pro')) {
            try {
                ...
                if (!empty($product_info->products[0])) {
                    ...
                    if (!Schema::hasColumn('infix_general_settings', $name)) {
                        Schema::table('infix_general_settings', function ($table) use ($name) {
                    ...
                    try {
                        ...
                        $config = InfixGeneralSetting::find(1);
                        $config->$name = 1;
                        $r = $config->save();
