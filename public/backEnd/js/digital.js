...
function submit_comment(id) {
    ...
    var data = $(`<textarea class="form-control textarea-reply comment{{ $errors->has('comment') ? ' is-invalid' : '' }}" name="comment" id="" cols="20" rows="3" placeholder="Reply here..."></textarea>
               <div class="form_btn d-flex justify-content-between"> <div class="file_upload"> <div class="input-group"> <div class="custom-file"> <input type="file" class="custom-file-input {{ $errors->has('file') ? ' is-invalid' : '' }} cursor_pointer" id="inputGroupFile04"
               aria-describedby="inputGroupFileAddon04" name="file"> <label class="custom-file-label" for="inputGroupFile04"> <svg class="upload_file svg-inline--fa fa-paperclip fa-w-14 icon mr-10" aria-hidden="true" data-prefix="fas" data-icon="paperclip" role="img"
                ...
                </path> </svg> Add Attachment</label> </div> </div> </div> <button class="primary-btn small fix-gr-bg submit_com current" type="submit">Comment reply</button> </div>`);
...
$(document).ready(function() {
    ...
    if (a) {
        $.ajax({
            type: "GET",
            url: url + "/ticket/admin/ticket-show/" + a,
            data: {
                id: a,
            },
            dataType: "json",
